...
public class MapPanel extends JPanel {
	...
	public void paintRoute() {
		...
		Color[] color = new Color[] { Color.CYAN, Color.GREEN, Color.MAGENTA,
				Color.ORANGE, Color.GRAY, Color.PINK, Color.YELLOW, Color.RED, Color.BLACK, Color.BLUE, Color.DARK_GRAY};
		float dash1[] = { 10.0f };
		...
		for (int i = 1; i < route.length; i++) {
			if (route[i - 1] == area.school) {
				...
				if (c == color.length) {
					c = 0;
					if (d == 0) g2.setStroke(dashed);
					else if (d == 1) g2.setStroke(new BasicStroke(2));
					d = (d == 0) ? 1 : 0;
				}
				g2.setPaint(color[c]);
			}
	...
	public void paint(Graphics g) {
		...
		for (int i = 0; i < stops.length; i++) {
			...
			if (i > 0) {
				int size = (int) Math
						.sqrt(((double) (32 * stops[i].numberOfPupils))
								/ Math.PI);
	...
	private Point project(BusStop stop, int diffX, int diffY, int xoffset,
			int yoffset) {
